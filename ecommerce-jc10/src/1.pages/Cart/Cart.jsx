...
class Cart extends Component {
    ...
    onBtnPay = () => {
        ...
        let newData= {
            ...
            kodepos: this.state.inputKodePos,
            alamat: this.state.inputAlamat
        }

        if(totalPrice>this.state.inputUang){
            swal('Uang anda tidak cukup','klo nga ada uang jngan belanja','warning')
        }else if(this.state.inputUang>totalPrice){
            swal('Berhasil Bayar','Uang Kembalian Anda adalah'+(this.state.inputUang-totalPrice),'success')
        ...
        for(var i=0;i<this.state.cartData.length;i++){
            ...
            swal('Uang anda pas','mkasih dah belanja yaa','success')
            Axios.post(urlApi+'history',newData)
            .then(res => {
               console.log(res)
            })
